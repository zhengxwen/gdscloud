...
- **Amazon S3** — URLs of the form `s3://bucket/key`; S3-compatible services
  (MinIO, Ceph RGW, Cloudflare R2, Wasabi, Backblaze B2, ...) via a custom endpoint
...
Any service implementing the S3 API (MinIO, Ceph RGW, Cloudflare R2, Wasabi,
Backblaze B2, DigitalOcean Spaces, ...) can be used by pointing the S3
backend at its endpoint. Path-style addressing (`https://host/bucket/key`)
is the default for a custom endpoint; `region` is still part of the
signature and should be set to what the service documents:

```{r auth-s3-compat}
# a MinIO or Ceph instance on the local network
gdsCloudConfigS3(
...
The endpoint can also be given through the `AWS_ENDPOINT_URL_S3` or
`AWS_ENDPOINT_URL` environment variable, as understood by the AWS CLI and
SDKs. Then open `s3://bucket/key` as usual:

```{r open-s3-compat, eval=FALSE}
...
An OAuth2 access token from Microsoft Entra ID (for example from
`az account get-access-token --resource https://storage.azure.com/`, or a
managed identity) can be used instead of a key or SAS token, and sovereign
clouds or local emulators are reached through `endpoint_suffix` or a full
`endpoint`:
...
gdsCloudConfigAzure(
    ...
    endpoint_suffix = "blob.core.chinacloudapi.cn"   # Azure China
)
# Azurite emulator
gdsCloudConfigAzure(
...
## Short-lived tokens: token provider functions

Google OAuth2 access tokens and Microsoft Entra ID tokens expire after
about an hour. Instead of a fixed string, the token arguments
(`access_token` for GCS and Azure, `bearer_token` for HTTP) accept a
function that returns the token; it is called each time a file is opened,
so a fresh token is fetched automatically. This is also the way to reuse
the login machinery of dedicated packages:

```{r auth-provider, eval=FALSE}
# Google Cloud Storage via gargle, or via the gcloud CLI
gdsCloudConfigGCS(access_token = function()
    gargle::token_fetch(scopes = "https://www.googleapis.com/auth/devstorage.read_only")$credentials$access_token)
gdsCloudConfigGCS(access_token = function()
...
AWS temporary credentials (STS, SSO, instance roles) are three values that
expire together, so for S3 a single `credentials` function returns the
whole set; the AWS CLI's `export-credentials` command resolves the complete
AWS credential chain:

```{r auth-provider-aws, eval=FALSE}
...
A function returning `NULL` or `""` means no token is available and the
next fallback is used. Provider functions are exported to parallel workers
by `gdsCloudExportCredentials()` like any other credential. Static keys
(AWS access keys, Azure account keys, SAS tokens) are given as strings.
